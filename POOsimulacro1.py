...
- recibir un ataque, en este caso se especifican los puntos de fuerza del ataque recibido. La Enterprise "detiene" el ataque con la coraza,
 y si la coraza no alcanza, el resto se descuenta de la potencia. Por ejemplo si la Enterprise con 80 de potencia y 12 de coraza recibe un ataque 
 de 20 puntos de fuerza, puede parar solamente 12 con la coraza, los otros 8 se descuentan de la potencia. La nave debe quedar con 72 de potencia
  y 0 de coraza. Si la Enterprise no tiene nada de coraza al momento de recibir el ataque, entonces todos los puntos de fuerza del ataque se 
  descuentan de la potencia.
La potencia y la coraza tienen que mantenerse en los rangos indicados, por ejemplo, si la Enterprise tien 16 puntos de coraza y se encuentra con
 un escudo, entonces queda en 20 puntos de coraza, no en 26. Tampoco puede quedar negativa la potencia, a lo sumo queda en 0. La Enterprise nace 
 con 50 de potencia y 5 de coraza. Implementar este modelo de la Enterprise, que tiene que entender los siguientes mensajes:
...
#siempre cuando haces un IF arranca poniendo al revez de como te digan para despues poner en el else lo que queda.
class enterprise():
	def __init__(self):
		self.potencia=50
		self.coraza()=5
	...
	def coraza(self):
		return self.coraza
	def encontrarPilaAtomica(self):  #si el problema es que si sumas 25 y queda mayor o igual a 100, pone que sea menor o igual asi demostras lo que tiene que pasar. y despues en ELSE pones lo que pasaria malo.
